...
from scratch. `list`, `init`, `add`, `remove`, and `doctor` are written; `diff` is specified but not.
The registry holds `button`, `badge`, `card`, `label`, `separator`, and the `utils` and `text`
helpers they import.
